 ...
 */
export default async function removeFavoriteOrder(context, input) {
  let prepTime = 0;
  let taxID = "";
  ...
  console.log("ORDER RECORD", favorteOrder)
  // const insertedFavoriteOrder = await FavoriteOrder.insertOne(favorteOrder);
  // console.log("insertedFavoriteOrder ", insertedFavoriteOrder)
  // Insert/update catalog document
  const deletedFavoriteOrder = await FavoriteOrder.deleteOne(
    {
      accountId,
      orderId,
    }
